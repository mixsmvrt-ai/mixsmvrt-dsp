...
def analyze_mono_signal(audio: np.ndarray, sr: int) -> Dict[str, float]:
    ...
    # Essentia-based implementation
    try:
        window = es.Windowing(type="hann")  # type: ignore[attr-defined]
        spectrum = es.Spectrum()  # type: ignore[attr-defined]
        centroid_alg = es.Centroid()  # type: ignore[attr-defined]
        rms_alg = es.RMS()  # type: ignore[attr-defined]
        ...
        logger.debug(
            "[DSP] Essentia analysis completed: rms=%.3f, bright=%.1f, trans=%.3f, dr=%.2f",
            rms,
            brightness,
            transients,
            dynamic_range,
