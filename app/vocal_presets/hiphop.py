...
def _process_vocal_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    ...
    plugins = [
        ...
        # Subtractive EQ – tighten low end before hard compression
        LowShelfFilter(
            cutoff_frequency_hz=180.0,
            gain_db=-0.5,
        ),
        # Level compressor – main peak/forwardness control
        Compressor(
            threshold_db=-22.0,
            ratio=5.2,
            attack_ms=4.0,
        ...
        # Additive EQ – mid presence and top air
        PeakFilter(
        ...
        # Glue compressor – slightly gentler, to sit the shaped tone in the mix
        Compressor(
            threshold_db=-19.0,
            ratio=2.7,
            attack_ms=14.0,
...
def _process_background_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    ...
    bg_plugins = [p for p in bg_plugins if p.__class__.__module__.startswith("pedalboard")]
...
def _process_adlib_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    ...
    adlib_plugins = [p for p in adlib_plugins if p.__class__.__module__.startswith("pedalboard")]
