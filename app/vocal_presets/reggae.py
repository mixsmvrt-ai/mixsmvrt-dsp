...
def _process_vocal_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    ...
    plugins = [
        ...
        # Subtractive EQ – keep reggae leads warm but remove excess rumble/mud
        LowShelfFilter(
...
def _process_background_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    """Reggae background vocals – wider, softer, more ambience."""
    if audio.size == 0:
        return audio
    ...
    bg_board = Pedalboard(  # type: ignore
        [
            ...
            HighShelfFilter(cutoff_frequency_hz=9500.0, gain_db=-0.5),
            Reverb(
                room_size=0.46,
                damping=0.56,
                wet_level=0.34,
...
def _process_adlib_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    """Reggae adlibs – echo‑y, bright accents sitting behind the lead."""
    if audio.size == 0:
        return audio
    ...
    adlib_board = Pedalboard(  # type: ignore
        [
            ...
            PeakFilter(cutoff_frequency_hz=2800.0, gain_db=2.0, q=1.0),
            HighShelfFilter(cutoff_frequency_hz=11000.0, gain_db=2.0),
            Saturation(drive_db=4.5),
            Reverb(
                room_size=0.52,
                damping=0.56,
                wet_level=0.38,
