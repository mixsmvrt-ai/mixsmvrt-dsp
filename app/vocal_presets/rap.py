...
def _process_vocal_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    ...
    highpass_cutoff = 105.0 if not is_female else 120.0
    ...
    plugins = [
        ...
        NoiseGate(threshold_db=-48.0, ratio=2.4, release_ms=130.0),
        Deesser(
            frequency=deesser_freq,
            threshold_db=-30.0,
            ratio=4.0,
        ),
        # Subtractive EQ – firm low‑end control plus a bit of mud cleanup
        LowShelfFilter(
            cutoff_frequency_hz=220.0,
            gain_db=-2.5,
        ),
        PeakFilter(
            cutoff_frequency_hz=320.0,
            gain_db=-2.5,
            q=1.1,
        ),
        PeakFilter(
            cutoff_frequency_hz=520.0,
            gain_db=-2.0,
            q=1.2,
        ),
        # Level compressor – very assertive peak/density stage
        Compressor(
            threshold_db=-23.5,
            ratio=5.8,
            attack_ms=4.0,
        ...
        # Additive EQ – upper‑mid cut-through and air
        PeakFilter(
        ...
        HighShelfFilter(
            cutoff_frequency_hz=11000.0,
            gain_db=highshelf_gain,
        ),
        # Glue compressor – a bit slower to pull everything together
        Compressor(
            threshold_db=-20.0,
            ratio=2.8,
            attack_ms=14.0,
...
def _process_background_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    """Rap background vocals – tucked, darker, still punchy."""
    if audio.size == 0:
        return audio
    ...
    bg_plugins: list[Any] = [
        ...
        LowShelfFilter(cutoff_frequency_hz=240.0, gain_db=-2.5),
...
def _process_adlib_gender(audio: np.ndarray, sr: int, gender: str | None) -> np.ndarray:
    """Rap adlibs – very hyped, bright and FX‑heavy."""
    if audio.size == 0:
        return audio
    ...
    adlib_plugins: list[Any] = [
        ...
        PeakFilter(cutoff_frequency_hz=3400.0, gain_db=3.5, q=1.0),
        HighShelfFilter(cutoff_frequency_hz=12500.0, gain_db=2.5),
